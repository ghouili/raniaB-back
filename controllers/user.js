...
const Register = async (req, res) => {
    ...
    const token = jwt.sign(payload, 'your-secret-key', { expiresIn: 3600 }); // Token expires in 1 hour

    return res.status(201).json({ success: true, message: 'user added successfully', data: NewUser, toke: token });
...
const Login = async (req, res) => {
    ...
    return res.status(200).json({ success: true, message: `Logged Successfully, Welcome Mr/Miss/Mrs ${existingUser.name}`, data: existingUser, token: token });
